...
// void ins_send_text_message(uint8_t command, char* format, ...) {
...
// void ins_report_error(char* format, ...) {
//     // If we are streaming, don't send anything
//     if(streaming != 0)
//         return;
...
void ins_process_line(char* data, uint32_t length) {
    ...
    uint8_t data_buffer[INS_BUFFER_LEN + 1];
...
void ins_read_next() {
    ...
    while(instruction_buffer[i_new_line] != '\n') {
        i_new_line++;
        if(i_new_line >= instruction_buffer_end) {
            //ins_invalidate_buffer();
            return; // If no new line was found, invalidate the buffer and return
        }
    }
